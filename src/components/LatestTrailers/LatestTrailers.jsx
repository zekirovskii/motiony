...
import { fetchTrailersByCategory , fetchVideosById } from "../../services/tmdbApi";
import css from "../LatestTrailers/LatestTrailer.module.css";
...
export default function LatestTrailers() {
  ...
  useEffect(() => {
    async function loadTrailers() {
      try {
        const data = await fetchTrailersByCategory(category);
        setTrailers(data);
      } catch (error) {
 ...
 const randomIndex = trailers.length > 0 
  ? Math.floor(Math.random() * trailers.length) 
  : 0;
    
const backgroundImage = trailers[randomIndex]?.backdrop_path
  ? `url(https://image.tmdb.org/t/p/original${trailers[randomIndex].backdrop_path})`
  : "none";
        ...
        <div className={css.container}>
          <div className={css.header}>
            ...
            <div className={css.tabs}>
              {["popular", "streaming", "on_tv", "for_rent", "in_theaters"].map((cat) => (
                <button
                  key={cat}
